...
namespace SixRens.式.三传实现
{
    internal partial class 三传实现
    {
        ...
        private void 还有三课(四课之一[] 四课, IReadOnlyList<四课之一> 不重复三课)
        {
...

            {
                ...
                if (日阴阳.IsYang)
                {
                    ...
                    this.初传 = 天地盘.取所乘神(日.Value.取所合干().寄宫());
                    return;
                }
        ...
        private IEnumerable<四课之一> 取上比者(IEnumerable<四课之一> 各课, YinYang 日阴阳)
        {
        ...
        private IReadOnlyList<四课之一> 上克下比较涉害程度取(IEnumerable<四课之一> 各课)
        {
            int 最大涉害程度 = 0;
            List<四课之一> 结果 = new(4);
            foreach (var 课 in 各课)
        ...
        private IReadOnlyList<四课之一> 下贼上比较涉害程度取(IEnumerable<四课之一> 各课)
        {
            int 最大涉害程度 = 0;
            List<四课之一> 结果 = new(4);
            foreach (var 课 in 各课)
        ...
        private IEnumerable<四课之一> 按下孟仲取(IEnumerable<四课之一> 各课)
        {
        ...
        private IReadOnlyList<四课之一> 取遥克(
